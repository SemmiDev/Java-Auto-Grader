...
func main() {
	...
	tokenMaker, err := helper.NewPasetoMaker(config.TokenSymmetricKey)
	if err != nil {
	...
	autoGraderDB := mongoDB.GetClient().Database(config.MongodbDatabaseName)
	...
	userSvc := userService.NewUserService(userRepo, tokenMaker, config)
	...
	s := NewServer(tokenMaker, config, userSvc, classSvc, submissionSvc)
	s.SetupMiddlewares()
	s.SetupRoutes()
